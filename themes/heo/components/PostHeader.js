...
export default function PostHeader({ post, siteInfo, isDarkMode }) {
  ...
  return (
    <div
      id='post-bg'
      className='md:mb-0 -mb-5 w-full h-[25rem] relative md:flex-shrink-0 overflow-hidden bg-cover bg-center bg-no-repeat z-10'>
      <style jsx>{`
        .coverdiv:after {
          position: absolute;
          content: '';
      ...
      <div
        className={`${isDarkMode ? 'bg-[#CA8A04]' : 'bg-[#1a202c]'} absolute top-0 w-full h-full py-10 flex justify-center items-center`}>
        {/* 文章背景图 */}
        <div
            ...
            {post.category && (
              <>
                <Link
                  ...
                  passHref
                  legacyBehavior>
                  <div className='cursor-pointer font-sm font-bold px-3 py-1 rounded-lg  hover:bg-white text-white bg-green-500 dark:bg-yellow-500 hover:text-blue-500 duration-200 '>
                    {post.category}
                  </div>
                </Link>
              </>
